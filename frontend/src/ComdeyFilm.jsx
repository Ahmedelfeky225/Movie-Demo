...
import { Link } from 'react-router-dom'; // استيراد Link من react-router-dom
  ...
  export function ComdeyFilm({limit=4,page=1}) {
    const [movies, setMovies] = useState([]);

    useEffect(() => {
      fetch(`http://localhost:8080/movies/category/670874a74d96e86e905c2e8a?page=${page}&limit=${limit}`)
    ...
    return (
      <div className="container-fluid mt-4 box-shadoww rounded-1" style={{ backgroundColor: "#fff" }}> {/* تعيين خلفية بيضاء */}
        <h6 className="text-start fw-bold mb-0" style={{padding:"12px 0 12px 4px",color:"rgb(76, 15, 189"}}>Comdey Movies</h6> {/* عنوان القسم */}
