...
export function ComedyFilm() {
  ...
  const ComdeyMovies = movies.filter(movie => movie.genres.includes("Comedy"));

  return (
    <div className="container-fluid mt-4 box-shadoww rounded-1" style={{backgroundColor: "#fff" }}> {/* تعيين خلفية بيضاء */}
      <h6 className="text-start fw-bold mb-0" style={{padding:"12px 0 12px 4px",color:"rgb(76, 15, 189)"}}>Comedy Movies</h6> {/* عنوان القسم */}
      <div className="row"> {/* شبكة الأفلام */}
        {ComdeyMovies.slice(0,4).map((movie) => (
